...
class Baimangu : ParsedHttpSource() {
    ...
    override fun popularMangaRequest(page: Int) =
        GET("$baseUrl/vodshow/4--hits------$page---.html", headers)
    ...
    override fun mangaDetailsParse(document: Document): SManga {
        ...
        return SManga.create().apply {
            ...
            genre = detailElements.select("li.fed-show-md-block:nth-last-child(2) a:not(:empty)")
    ...
    override fun setPreferenceScreen(screen: androidx.preference.PreferenceScreen) {
        val mainSiteUrlPreference = androidx.preference.EditTextPreference(screen.context).apply {
            ...
            setOnPreferenceChangeListener { _, newValue ->
                try {
                    val setting =
                        preferences.edit().putString(MAINSITE_URL_PREF, newValue as String).commit()
                ...
                setOnPreferenceChangeListener { _, newValue ->
                    try {
                        val setting = preferences.edit()
                            .putString(MAINSITE_RATEPERMITS_PREF, newValue as String).commit()
                        Toast.makeText(screen.context, TOAST_RESTART, Toast.LENGTH_LONG).show()
                        setting
                    } catch (e: Exception) {
                        e.printStackTrace()
        ...
        val mainSiteRatePeriodPreference =
            androidx.preference.ListPreference(screen.context).apply {
                ...
                setOnPreferenceChangeListener { _, newValue ->
                    try {
                        val setting = preferences.edit()
                            .putString(MAINSITE_RATEPERIOD_PREF, newValue as String).commit()
                        Toast.makeText(screen.context, TOAST_RESTART, Toast.LENGTH_LONG).show()
                        setting
                    } catch (e: Exception) {
                        e.printStackTrace()
    ...
    companion object {
        ...
        private const val MAINSITE_URL_PREF_SUMMARY =
        ...
        private const val MAINSITE_RATEPERMITS_PREF_SUMMARY =
            "此值影响主站的连接请求量。需要重启软件以生效。\n默认值：$MAINSITE_RATEPERMITS_PREF_DEFAULT \n当前值：%s"
        private val MAINSITE_RATEPERMITS_PREF_ENTRIES_ARRAY =
            (1..10).map { i -> i.toString() }.toTypedArray()
        ...
        private const val MAINSITE_RATEPERIOD_PREF_SUMMARY =
            "此值影响主站点连接限制时的延迟（毫秒）。需要重启软件以生效。\n默认值：$MAINSITE_RATEPERIOD_PREF_DEFAULT\n当前值：%s"
        private val MAINSITE_RATEPERIOD_PREF_ENTRIES_ARRAY =
            (500..6000 step 500).map { i -> i.toString() }.toTypedArray()
    }
}
